...
def run_static_noise_experiment():
    ...
    # Import required modules inside function to avoid issues
    ...
    def static_noise_tesselation(N):
        ...
        class StaticNoiseTessellation:
            def __name__(self):
                return "static_noise_tesselation"
        return StaticNoiseTessellation()
...
def check_all_samples_exist(N, theta, deviation_ranges, samples, steps):
    ...
    for dev in deviation_ranges:
        ...
        exp_dir = get_experiment_dir(
            lambda n: None, has_noise, N, noise_params, "static_noise", "experiments_data_samples"
        )
        exp_dir = exp_dir.replace("None_", "static_noise_tesselation_")
...
def run_static_noise_samples_experiment(N, theta, steps, init_nodes, deviation_ranges, samples):
    ...
    for dev_idx, deviation in enumerate(tqdm(deviation_ranges, desc="Deviations")):
        ...
        # Fix directory name to use proper tessellation name
        exp_dir = exp_dir.replace("None_", "static_noise_tesselation_")
...
# Import the analysis functions from the original file
