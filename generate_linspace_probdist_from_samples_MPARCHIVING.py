...
ARCHIVE_LOG_FILE = os.path.join("logs", current_date, "generate_probdist_linspace", "archiving_process.log")
...
def setup_archive_logging():
    """Setup logging for the archiving process"""
    # Create archive logger
    archive_logger = logging.getLogger("archive")
    archive_logger.setLevel(logging.INFO)
    
    # Remove any existing handlers
    for handler in archive_logger.handlers[:]:
        archive_logger.removeHandler(handler)
    
    # Ensure the directory exists for archive log file
    archive_log_dir = os.path.dirname(ARCHIVE_LOG_FILE)
    if archive_log_dir:
        os.makedirs(archive_log_dir, exist_ok=True)
    
    # Create file handler
    file_handler = logging.FileHandler(ARCHIVE_LOG_FILE, mode='w')
    file_handler.setLevel(logging.INFO)
    ...
    console_formatter = logging.Formatter('[ARCHIVE] %(message)s')
...
def generate_probdist_for_dev_chunk(chunk_args):
    ...
    try:
        ...
        # === PER-CHUNK ARCHIVE CREATION ===
        chunk_tar_path = None
        if CREATE_TAR and processed_devs > 0:
            import tarfile
            os.makedirs(ARCHIVE_DIR, exist_ok=True)
            
            # Format chunk range for filename
            if len(dev_chunk) > 0:
                first_dev = dev_chunk[0][1] if isinstance(dev_chunk[0], tuple) else dev_chunk[0]
                last_dev = dev_chunk[-1][1] if isinstance(dev_chunk[-1], tuple) else dev_chunk[-1]
                chunk_range_str = f"chunk{process_id}_dev{first_dev:.3f}to{last_dev:.3f}"
            else:
                chunk_range_str = f"chunk{process_id}_empty"
            
            # Create archive for this chunk
            chunk_tar_name = f"experiments_data_probDist_linspace_N{N}_samples{samples_count}_theta{theta_param:.6f}_{chunk_range_str}.tar"
            chunk_tar_path = os.path.join(ARCHIVE_DIR, chunk_tar_name)
            
            try:
                with tarfile.open(chunk_tar_path, "w") as tar:
                    # Archive the probdist directories for all devs in this chunk
                    for dev in dev_chunk:
                        # Format deviation for directory name
                        if isinstance(dev, (tuple, list)) and len(dev) == 2:
                            dev_min, dev_max = dev
                        ...
                        if os.path.exists(probdist_exp_dir):
                            # Add the deviation directory to the archive
                            arcname = os.path.join("static_noise_linspace", f"N_{N}", theta_folder, dev_folder)
                            tar.add(probdist_exp_dir, arcname=arcname)
                
                logger.info(f"Created chunk archive: {chunk_tar_path}")
                
            except Exception as e:
                logger.error(f"Failed to create chunk archive: {e}")
                chunk_tar_path = None
        ...
        if chunk_tar_path:
            logger.info(f"Chunk archive: {chunk_tar_path}")
...
def main():
        ...
        with ProcessPoolExecutor(max_workers=len(process_args)) as executor:
            ...
            for future in as_completed(future_to_process, timeout=PROCESS_TIMEOUT * len(process_args)):
                ...
                try:
                    ...
                    if result["success"]:
                        archive_info = f", Archive: {os.path.basename(result['chunk_tar_path'])}" if result.get('chunk_tar_path') else ", No archive"
                        master_logger.info(f"Process {process_id} completed successfully: "
                                         ...
                                         f"time: {result['total_time']:.1f}s{archive_info}")
                    else:
                    ...
                    process_results.append({
                        ...
                        "total_steps": 0, "total_time": PROCESS_TIMEOUT, "chunk_tar_path": None
                    })
                except Exception as e:
                    ...
                    process_results.append({
                        ...
                        "processed_devs": 0, "computed_steps": 0, "skipped_steps": 0,
                        "total_steps": 0, "total_time": 0, "chunk_tar_path": None
                    })
    ...
    if CREATE_TAR:
        # Setup archive logging
        archive_logger = setup_archive_logging()
        archive_logger.info("=== ARCHIVING PROCESS STARTED ===")
        archive_logger.info(f"Archive directory: {ARCHIVE_DIR}")
        ...
        # Collect chunk tar paths from successful processes
        chunk_tar_paths = [r.get("chunk_tar_path") for r in process_results if r.get("chunk_tar_path")]
        
        archive_logger.info(f"Found {len(chunk_tar_paths)} chunk archives to combine")
        archive_logger.info(f"Creating main archive: {main_tar_path}")
        
        print(f"Creating main archive from {len(chunk_tar_paths)} chunk archives: {main_tar_path}")
        master_logger.info(f"Creating main archive from {len(chunk_tar_paths)} chunk archives: {main_tar_path}")
        
        try:
            if chunk_tar_paths:
                # Method 1: Combine chunk archives into main archive
                archive_logger.info("Using chunk-based archiving method")
                with tarfile.open(main_tar_path, "w") as tar:
                    for i, chunk_tar in enumerate(chunk_tar_paths):
                        archive_logger.info(f"Adding chunk archive {i+1}/{len(chunk_tar_paths)}: {os.path.basename(chunk_tar)}")
                        tar.add(chunk_tar, arcname=os.path.basename(chunk_tar))
                
                # Delete temporary chunk tar files
                archive_logger.info("Cleaning up temporary chunk archives")
                for chunk_tar in chunk_tar_paths:
                    try:
                        os.remove(chunk_tar)
                        archive_logger.info(f"Deleted temporary archive: {os.path.basename(chunk_tar)}")
                    except Exception as e:
                        archive_logger.warning(f"Could not delete {chunk_tar}: {e}")
            else:
                # Method 2: Fallback to direct directory archiving
                archive_logger.info("No chunk archives found, using direct directory archiving")
                with tarfile.open(main_tar_path, "w") as tar:
            ...
            archive_logger.info(f"Successfully created main archive: {main_tar_path}")
            
        except Exception as e:
            error_msg = f"Failed to create archive: {e}"
            master_logger.error(error_msg)
            archive_logger.error(error_msg)
            archive_logger.error(traceback.format_exc())
        
        archive_logger.info("=== ARCHIVING PROCESS COMPLETED ===")
        
    else:
        master_logger.info("Archiving disabled (CREATE_TAR=False)")
    ...
    print(f"Log files: {MASTER_LOG_FILE}, {ARCHIVE_LOG_FILE}, {PROCESS_LOG_DIR}/")
    ...
    master_logger.info(f"Log files: Master: {MASTER_LOG_FILE}, Archive: {ARCHIVE_LOG_FILE}, Process: {PROCESS_LOG_DIR}/")
    ...
    for result in process_results:
        if result["success"]:
            ...
            master_logger.info(f"  Process {result['process_id']}: SUCCESS - "
                             ...
                             f"Time: {result.get('total_time', 0):.1f}s{archive_info}")
        else:
