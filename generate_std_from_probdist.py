...
def setup_process_logging(dev_value, process_id, theta=None):
    ...
    log_filename = os.path.join(PROCESS_LOG_DIR, f"process_dev_{dev_str}{theta_str}_std.log")
...
def generate_std_for_dev(dev_args):
    ...
    try:
        ...
        if CREATE_TAR:
            ...
            dev_tar_path = os.path.join(ARCHIVE_DIR, f"std_{theta_folder_name}_theta{theta_param:.6f}_{devstr}.tar")
...
def main():
    ...
    if CREATE_TAR:
        ...
        main_tar_name = f"experiments_data_std_N{N}_samples{samples}_theta{theta:.6f}_{timestamp}.tar"
